...
data <- setDT(readRDS("/home/j/WORK/11_geospatial/02_processed data/U5M/Global/CBH_with_SBH_1988_2017.RDS"))
info <- fread("/share/geospatial/royburst/sbh_cbh/data/ihme_lc_info.csv")
...
data <- subset(data,! nid %in% 20786) # There seems to be some issue with this particular dataset
...
write.csv(cs, file = sprintf('%s/cs.csv',rdc_dir))
