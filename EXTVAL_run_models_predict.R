...
best_rd <- as.character(read.table(sprintf('%s/dated_sbh_model_fitting/best_run_date.txt',root))[1,1])
message(sprintf('BEST RUN_DATE OF FITTED MODEL TO BE USED: %s', best_rd))

# references to some key directories needed here
mod_dir     <- sprintf('%s/dated_sbh_model_fitting/%s', root, best_rd)
rd_dir      <- sprintf('%s/dated_sbh_model_prediction/%s', root, run_date)
...
# ab times
ab_times <- fread(sprintf('%s/ab_times.csv',mod_dir))
...
#data <- setDT(readRDS(sprintf('%s/SBH_TEST.RDS',dat_dir))) # FOR TESTING
data <- setDT(readRDS(sprintf('%s/Global_SBH_1988_2017.RDS',dat_dir)))
# NOTE SAVE A SAMPLE DATASET FOR TESTING SO TO NOT LOAD FULL DATA

# subset to survey
d <- subset(data, nid == snid)
rm(data)

# deal with subnational ihme ids. Dont need them, so turn them into country codes
d[, country := substr(country,1,3)]

# load in metadata for the countries to be able to get region
# HELPFUL: https://shiny.ihme.washington.edu/connect/#/apps/88/access
# HELPFUL: http://www.fao.org/countryprofiles/iso3list/en/
info <- fread(sprintf('%s/inputs/ihme_lc_info.csv',root)) 
...
saveRDS(d, sprintf('%s/raw_survey_data.RDS',rd_nid_dir))

# load in and properly label sdi info, important covariate for later
sdi <-fread(sprintf('%s/inputs/sdi.csv',root)) [,c('location_id','year_id','sdi'),with=FALSE]
...
cs <- fread(sprintf('%s/%s/cs.csv', mod_dir, region))
...
mod <- readRDS(sprintf('%s/%s/model_object.RDS', mod_dir, region))
...
## #######################################################
## IF THE SURVEY IS LARGE THE REST OF THE SCRIPT MUST HAPPEN IN CHUNKS TO AVOID THE OOM KILLER
...
for(sub in 1:max(d_orig$subset)){
  ...
  # Backfill SDI, not a big deal here because 1970 was so long ago it wont appear in our final dataset
  ...
  load(sprintf('%s/inputs/MAP.rdata',root))
  ...
  saveRDS(aggres, sprintf('%s/prepped_yearly_aggregation_summary_%i.RDS',rd_nid_dir,sub))
  
  # SAVE FINAL AGGREGATED YEARLY DRAW-LEVEL DATA
  saveRDS(draws_agg, sprintf('%s/prepped_yearly_aggregation_draws_%i.RDS',rd_nid_dir,sub))
  rm(draws_agg) # clear from mem
  
  # plot out the results just for fun
  pdf(sprintf('%s/geo_unit_time_trends_%i.pdf',rd_nid_dir,sub))
